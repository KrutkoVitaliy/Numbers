...
public class Root extends Base implements IOnAreaTouchListener, IOnSceneTouchListener
{
	int Seconds, Minutes, Hours, Correct, Incorrect;
	private Text tInput, tFirst, tTitle;
	...
	private void Environment()
	{
		...
		tTitle = new Text(120, 80, _Resources._OswaldLightLargeF, "Кв. корень", _VBO);
	...
	private void Generate()
	{
		...
		iFirst = (int) Math.pow((int) (Math.random() * (Correct)), 2);
		iResult = (int) Math.sqrt(iFirst);
		
		tFirst.setText("√"+iFirst);
		
		tFirst.setPosition(350 - tFirst.getWidth()/2, 350);
	}
		...
		final Rectangle Button = new Rectangle(x, y, w, h, _VBO)
		{
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY)
			{
				if(pSceneTouchEvent.isActionDown())
				{
					...
					{
						Uri URL = Uri.parse("https://ru.wikipedia.org/wiki/%D0%9A%D0%B2%D0%B0%D0%B4%D1%80%D0%B0%D1%82%D0%BD%D1%8B%D0%B9_%D0%BA%D0%BE%D1%80%D0%B5%D0%BD%D1%8C");
	...
	@Override public SceneType getSceneType(){return SceneType.ROOT;}
