...
public class Limits extends Base implements IOnAreaTouchListener, IOnSceneTouchListener
{
	private int Seconds, Minutes, Hours, Correct, Incorrect, Mode;
	private Text tTitle;
	...
	private void Environment()
	{
		...
		tTitle = new Text(120, 80, _Resources._OswaldLightLargeF, "Пределы", _VBO);
		tTitle.setHorizontalAlign(HorizontalAlign.CENTER);
		attachChild(tTitle);
		
		Task1 = new Sprite(40, 350, _Resources.Calc[0], _Resources._VBO);
		Task2 = new Sprite(40, 350, _Resources.Calc[1], _Resources._VBO);
		Task3 = new Sprite(40, 350, _Resources.Calc[2], _Resources._VBO);
		...
		Result1 = new Rectangle(38, 884, 640, 70, _VBO)
		{
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY)
			{
				...
				{
					Uri URL = Uri.parse("http://lifexperience.ru/Numbers/Limits/1_1.PNG");
		...
		Result2 = new Rectangle(38, 884, 640, 70, _VBO)
		{
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY)
			{
				if(pSceneTouchEvent.isActionDown())
				{
					Uri URL = Uri.parse("http://lifexperience.ru/Numbers/Limits/1_2.PNG");
					Intent LINK = new Intent(Intent.ACTION_VIEW, URL);
					_Activity.startActivity(LINK);
					return true;
				}
		...
		Result3 = new Rectangle(38, 884, 640, 70, _VBO)
		{
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY)
			{
				if(pSceneTouchEvent.isActionDown())
				{
					Uri URL = Uri.parse("http://lifexperience.ru/Numbers/Limits/1_3.PNG");
					Intent LINK = new Intent(Intent.ACTION_VIEW, URL);
					_Activity.startActivity(LINK);
					return true;
				}
	...
	private void Button(int x, int y, int w, int h, final float v)
	{
		...
		final Rectangle Button = new Rectangle(x, y, w, h, _VBO)
		{
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY)
			{
				if(pSceneTouchEvent.isActionDown())
				{
					...
					{
						Uri URL = Uri.parse("https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%B5%D0%B4%D0%B5%D0%BB_%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8");
						Intent LINK = new Intent(Intent.ACTION_VIEW, URL);
						_Activity.startActivity(LINK);
					}
					else if(v == 0)
	...
	@Override public SceneType getSceneType(){return SceneType.LIMITS;}
